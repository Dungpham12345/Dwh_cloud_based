...
resource "kubernetes_secret" "datahub_credentials" {
  count = var.enable_gke ? 1 : 0
  metadata {
    name = "datahub-login-secret"
  }
  data = {
    "SPRING_AUTH_USERNAME" = base64encode("admin")   # Tên người dùng
    "SPRING_AUTH_PASSWORD" = base64encode("admin123")  # Mật khẩu
  }
  type = "Opaque"
}
...
resource "helm_release" "datahub" {
  ...
  chart      = "datahub"
  namespace  = "default"
  depends_on = [helm_release.datahub_prereq, kubernetes_secret.datahub_credentials]

  set_sensitive {
    name  = "datahub.auth.username"
    value = "datahub"
  }

  set_sensitive {
    name  = "datahub.auth.password"
    value = "datahub"
  }
  ...
  set {
    name  = "elasticsearch.imageTag"
    value = "7.10.2"
  }

  set {
    name  = "datahub.elasticsearch.host"
...
# Đặt IP range dành riêng cho Google services
...
# Kết nối mạng với dịch vụ Google (Cloud SQL, etc.)
