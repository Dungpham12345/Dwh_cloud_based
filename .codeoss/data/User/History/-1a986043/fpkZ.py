...
def create_upload_tasks(bucket_name, project_id, dataset_name, dag, gcs_prefix="data/dwh"):
    ...
    for blob_path in gcs_files:
        table_name = os.path.basename(blob_path).split("_")[0]
...
with DAG(
    ...
    upload_all_tables = PythonOperator(
    ...
    # Step 2: Tạo task load từng file .avro → BigQuery
    ...
    test_conn >> extract_data >> upload_all_tables >> [transform_job, transform_employee, transform_department, transform_candidate, transform_candidate_log] >> save_transformed >> create_dataset_task >> 