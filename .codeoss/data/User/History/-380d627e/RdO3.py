"""
DAG: extract_and_transform_data_from_postgres
Tối ưu hoá ETL DAG với TaskGroup, vòng lặp cho transform/load, không import file cũ.
"""
    ...
    for _, row in table_df.iterrows():
        ...
        for col in df_copy.columns:
            if pd.api.types.is_datetime64_any_dtype(df_copy[col]):
                df_copy[col] = df_copy[col].apply(lambda x: int(pd.Timestamp(x).timestamp() * 1_000_000) if pd.notnull(x) else None)
...
def get_avro_schema(df, table_name):
    dtype_to_avro = {
        'object': 'string', 'int64': 'long', 'float64': 'double', 'bool': 'boolean',
...
def save_dim_and_fact_tables_to_gcs(bucket=BUCKET_NAME, output_dir="./output", gcs_prefix="data/dwh"):
    ...
    def save_upload(df: pd.DataFrame, table_name: str):
        # Loại NaN/null, convert datetime, sinh schema avro
        original_types = df.dtypes.astype(str).to_dict()
        for col in df.columns:
            if original_types[col] == 'object':
                df[col] = df[col].astype(str).replace({'nan': None, 'None': None, '': None})
