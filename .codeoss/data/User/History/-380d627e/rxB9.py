...
def extract_all_and_upload_to_gcs(bucket_name, schema_name="hrms", output_dir="./output", gcs_prefix="data/staging"):
    ...
    if df.empty and table != 'query_log':
        return
        ...
        for col in df_copy.columns:
            if pd.api.types.is_datetime64_any_dtype(df_copy[col]) or isinstance(df_copy[col].iloc[0], datetime):
                df_copy[col] = df_copy[col].apply(
                    lambda x: x.strftime('%Y-%m-%dT%H:%M:%S') if isinstance(x, datetime) and pd.notnull(x) else None)
...
def transform_query_log_table():
    df = extract_data_postgres(connect_to_postgres_hook(), 'query_log', 'hrms')
...
def save_dim_and_fact_tables_to_gcs(bucket=BUCKET_NAME, output_dir="./output", gcs_prefix="data/dwh"):
    ...
    def save_upload(df: pd.DataFrame, table_name: str):
        ...
        for col in df_copy.columns:
            if pd.api.types.is_datetime64_any_dtype(df_copy[col]) or (
                ...
                def safe_ts_micro(x):
                    try:
                        return int(pd.Timestamp(x).timestamp() * 1_000_000) if pd.notnull(x) else None
                    except Exception:
                        return None
                df_copy[col] = df_copy[col].apply(safe_ts_micro)

        schema = get_avro_schema(df_copy, table_name)
        records = df_copy.to_dict(orient="records")
    ...
    save_upload(transform_query_log_table(), 'query_log')
...
with DAG(
    ...
    with TaskGroup("ods_load_tasks_group") as ods_load_tasks:
        for blob_path in list_gcs_avro_files(BUCKET_NAME, prefix=f"data/staging/{SCHEMA_NAME}"):
            full_stem = Path(blob_path).stem  # e.g., query_log_20250617
            if full_stem.endswith(today_str):  # tách đuôi ngày ra
                table = full_stem[:-(len(today_str) + 1)]
            else:
                table = full_stem  # fallback
    ...
    transform_funcs = {
        ...
        'query_log': transform_query_log_table,
    }
    ...
    with TaskGroup("dwh_load_tasks_group") as dwh_load_tasks:
        for blob_path in list_gcs_avro_files(BUCKET_NAME, prefix="data/dwh"):
            full_stem = Path(blob_path).stem
            table = "_".join(full_stem.split("_")[:-1])
            GCSToBigQueryOperator(
