...
def transform_and_append(project_id: str, temp_dataset: str, dwh_dataset: str, today_suffix: str):
    today_suffix = datetime.today().strftime("%Y%m%d")
    prefix_len   = len(today_suffix) + 1  # including the underscore

    append_queries = [
        f"""
        INSERT INTO `{project_id}.{dwh_dataset}.dim_job_{today_suffix}`
        ...
        SELECT 
          ...
          start_date,
          end_date
        FROM `{project_id}.{temp_dataset}.temp_job_description_{today_suffix}`;
        """,

        f"""
        INSERT INTO `{project_id}.{dwh_dataset}.dim_employee_{today_suffix}`
        SELECT * FROM `{project_id}.{temp_dataset}.temp_employee_{today_suffix}`;
        """,

        f"""
        INSERT INTO `{project_id}.{dwh_dataset}.dim_department_{today_suffix}`
        SELECT * FROM `{project_id}.{temp_dataset}.temp_department_{today_suffix}`;
        """,

        f"""
        INSERT INTO `{project_id}.{dwh_dataset}.dim_candidate_{today_suffix}`
        ...
        SELECT 
          ...
          reason,
          apply_date
        FROM `{project_id}.{temp_dataset}.temp_candidates_{today_suffix}`;
        """,

        f"""
        INSERT INTO `{project_id}.{dwh_dataset}.fact_candidate_log_{today_suffix}`
        SELECT 
          a.*,
          CASE 
        ...
        FROM (
          SELECT 
            ...
            af.note
          FROM `{project_id}.{temp_dataset}.temp_applyfor_{today_suffix}` af
        ) a;
        """,

        f"""
        INSERT INTO `{project_id}.{dwh_dataset}.fact_pic_tracking_{today_suffix}`
        SELECT
          pic.employee_code,
          pic.opening_id,
          jd.status,
        ...
        FROM `{project_id}.{temp_dataset}.temp_person_in_charge_{today_suffix}` pic
        LEFT JOIN (
            SELECT
              opening_id,
            ...
            FROM `{project_id}.{temp_dataset}.temp_applyfor_{today_suffix}`
            GROUP BY opening_id
        ) js ON pic.opening_id = js.opening_id
        LEFT JOIN `{project_id}.{temp_dataset}.temp_job_description_{today_suffix}` jd ON pic.opening_id = jd.opening_id;
        """
    ]
