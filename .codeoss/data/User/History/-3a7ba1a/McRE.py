...
def transform_and_append(project_id: str, temp_dataset: str, dwh_dataset: str, today_suffix: str):
    ...
    append_queries = [
        ...
        f"""
        INSERT INTO `{project_id}.{dwh_dataset}.fact_candidate_log_20250616_{today_suffix}`
        SELECT 
          a.*,
          CASE 
        ...
        FROM (
          SELECT 
            ...
            af.note
          FROM `{project_id}.{temp_dataset}.temp_applyfor_20250616` af
        ) a;
        """,
        ...
        FROM `{project_id}.{temp_dataset}.temp_person_in_charge_20250616` pic
        LEFT JOIN (
            SELECT
              opening_id,
            ...
            FROM `{project_id}.{temp_dataset}.temp_applyfor_20250616`
            GROUP BY opening_id
        ) js ON pic.opening_id = js.opening_id
        LEFT JOIN `{project_id}.{temp_dataset}.temp_job_description_20250616` jd ON pic.opening_id = jd.opening_id;
        """
    ]
