...
def transform_and_append(project_id: str, temp_dataset: str, dwh_dataset: str):
    ...
    append_queries = [
        ...
        SELECT 
          ...
          start_date,
          end_date
        FROM `{project_id}.{temp_dataset}.temp_job_description_20250616`;
        """,

        f"""
        INSERT INTO `{project_id}.{dwh_dataset}.dim_employee_20250616`
        SELECT * FROM `{project_id}.{temp_dataset}.temp_employee_20250616`;
        """,

        f"""
        INSERT INTO `{project_id}.{dwh_dataset}.dim_department_20250616`
        SELECT * FROM `{project_id}.{temp_dataset}.temp_department_20250616`;
        """,
        ...
        SELECT 
          ...
          reason,
          apply_date
        FROM `{project_id}.{temp_dataset}.temp_candidates_20250616`;
        """,
